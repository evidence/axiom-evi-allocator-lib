...
typedef uint32_t axiom_region_flags_t;
typedef int      axiom_region_prio_t;

/** \brief Free list node */
struct axiom_freelist_node {
	/** Next free list node */
	struct axiom_freelist_node *next;
	/** Size of the free area */
	size_t size;
};

/** \brief Region descriptor */
typedef struct axiom_region_desc {
	/** Pointer to the following region */
	struct axiom_region_desc *next;
	
	/** List of free memory blocks in this region. */
	struct axiom_freelist_node *nodes;
...
typedef struct axiom_lmm {
	...
	axiom_region_desc_t *regions;
	/** Private buffer used to store "unnamed" regions */
	char workspace[4096 - sizeof(axiom_region_desc_t *)];
} axiom_lmm_t;

/* TODO: check power of 2???*/
#define AXIOM_LMM_ALIGN_SIZE      sizeof(struct axiom_freelist_node)
